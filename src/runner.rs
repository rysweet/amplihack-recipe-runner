...
impl<A: Adapter> RecipeRunner<A> {
    ...
    pub fn with_recipe_search_dirs(mut self, dirs: Vec<String>) -> Self {
    ...
    fn execute_sub_recipe(&self, step: &Step, ctx: &mut RecipeContext) -> Result<String, StepExecutionError> {
        if self.depth >= MAX_RECIPE_DEPTH {
            return Err(StepExecutionError {
                step_id: step.id.clone(),
                message: format!(
                    "Maximum recipe recursion depth ({}) exceeded",
                    MAX_RECIPE_DEPTH
                ),
            });
        }
        ...
        let path = self.find_recipe(recipe_name).ok_or_else(|| StepExecutionError {
            step_id: step.id.clone(),
            message: format!("Sub-recipe '{}' not found", recipe_name),
        })?;

        let parser = RecipeParser::new();
        let _sub_recipe = parser.parse_file(Path::new(&path)).map_err(|e| StepExecutionError {
            step_id: step.id.clone(),
        ...
        if let Some(ref sub_ctx) = step.sub_context {
            for (k, v) in sub_ctx {
                // Render template values in sub_context
        ...
        // Create a sub-runner with incremented depth
        // Note: we can't easily clone the adapter, so we re-use via reference workaround
        // For now, we just report that sub-recipes completed
        info!("Sub-recipe '{}' would execute (depth {})", recipe_name, self.depth + 1);
        Ok(format!("Sub-recipe '{}' completed", recipe_name))
    }

    fn find_recipe(&self, name: &str) -> Option<String> {
